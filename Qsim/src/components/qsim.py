...
arg_list = ['bgjob', 'cjob', 'config_file', 'outputlog', 'sleep_interval', 
            ...
            'Anchor', 'anchor', 'vicinity', 'mate_ratio', 'batch', 'power', 'power_budget', 'refill']
...
def integrated_main(options):
    ...
    #insert energy cost events
    #print "Inserting power monitor events into event list..."
    #evsim.init_power_monitor_events()
    ...
    # initialize last schedule time
    evsim.init_last_schedule_time()
    ...
    #print energy cost and log power consumption
    evsim.print_power_consumption_cost()
    evsim.print_schedule_info()
    evsim.log_power_consumption()
       
    endtime_sec = time.time()
    if opts.bgjob:
        print "Backfilling count:", bqsim.get_backfill_count()
...
if __name__ == "__main__":
    ...
    p.add_option("-l", "--pow", dest="power", action = "store_true", default = False,
        help="enable power-aware scheduling which aims to minimize the energy cost")
    p.add_option("-k", "--powbg", dest="power_budget", type="float", default=1500.0,
        help="Power budget used by power-aware scheduling")
    p.add_option("-w", "--refill", dest="refill", action="store_true", default = False,
        help="Window size used by power-aware scheduling")
