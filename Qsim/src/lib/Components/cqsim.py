 ...
 #       self.invisible_job_dict = {}   # for jobs not submitted, {jobid:job_instance}
    ...
    def init_queues(self):
        ...
        for key in raw_jobs:
            ...
            #average power
            spec['power'] = tmp.get('power', random.randint(10,33))
...
#####coscheduling stuff
    ...
    # power-aware functions
    def get_running_job_power_usage(self):
        total_power = 0
        
        for runningjob in self.running_jobs:
            power = int(runningjob.power)
            nodes = int(runningjob.nodes)
...
#                rack = node / 1024
    ...
    def get_idle_nodes(self):
        return self.total_job - self.num_busy
    
    def release_nodes(self, location):
        return self.nodes_up(location)
    release_nodes = exposed(release_nodes)