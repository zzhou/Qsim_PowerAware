...
class BGQsim(Simulator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # initialize the random seed for generating power consumption
        random.seed(1)
        self.power_aware = kwargs.get("power", False)
        self.power_budget = kwargs.get("power_budget", 1500)
        self.refill = kwargs.get("refill", False)
    ...
    def log_job_event(self, eventtype, timestamp, spec):
        ...
        if eventtype == 'Q':  #submitted(queued) for the first time
            ...
            if eventtype == 'S':  #start running 
                message = "%s;S;%s;queue=%s qtime=%s Resource_List.nodect=%s Resource_List.walltime=%s start=%s exec_host=%s power=%d" % \
                (timestamp, spec['jobid'], spec['queue'], spec['submittime'],
                 spec['nodes'], log_walltime, spec['start_time'], ":".join(spec['location']), spec['power'])
...
##### job/queue related
    ...
    def init_queues(self):
        ...
        for key in raw_jobs:
            ...
            #average power
            spec['power'] = tmp.get('power', random.randint(MIN_UNIT_POWER, MAX_UNIT_POWER))
            
            #print "Average power: ", spec['power'] 
    ...
    def get_norm(self, mu, sigma, max_n, min_n):
        while True:
    ...
    def find_job_location(self, arg_list, end_times):
        ...
        for job in arg_list:
            ...
            if partition_name:
                ...
                # reserve partition
                partitions = partition_name[job['jobid']]
                for partition in partitions:
                    self.reserve_partition(partition)
                break
            ...
            if location is not None:
                ...
                #print self.get_current_time_date(), "job %s is draining %s" % (job['jobid'], location.name)
        ...
        if not best_partition_dict:
		    ...
		    self.backfill_count += 1
                    ...
                    # reserve partition
                    partitions = partition_name[args['jobid']]
                    for partition in partitions:
                        self.reserve_partition(partition)
                    break
...
#            print "hig priority waiting jobs:"
    ...
    def get_running_job_power_usage(self):
        ...
        for runningjob in self.running_jobs:               
            total_power += get_power(runningjob)        
            
        return total_power
    get_running_job_power_usage = exposed(get_running_job_power_usage)

    def is_power_aware(self):
        return self.power_aware
    is_power_aware = exposed(is_power_aware)
    
    def get_running_job_number(self):
    ...
    def get_busy_nodes(self):
        return self.num_busy
    get_busy_nodes = exposed(get_busy_nodes)
    ...
    get_waiting = exposed(get_waiting)
    
    def get_power_budget(self):
        return self.power_budget
    get_power_budget = exposed(get_power_budget)
    
    def get_rest_power_budget(self):
        return self.power_budget - self.get_running_job_power_usage()
    get_rest_power_budget = exposed(get_rest_power_budget)
    
    def get_partition_size(self, partition_name):
        partition_size = 0
        
        for partition in partition_name:
            partition_size += self._partitions[partition].size
        
        return partition_size
    get_partition_size = exposed(get_partition_size)
        
    def get_refill(self):
        return self.refill
    get_refill = exposed(get_refill)       
    
    def get_sim_start(self):
        tmp = datetime.fromtimestamp(self.sim_start)
        fmtdate = tmp.strftime("%Y_%m_%d")
        return fmtdate
    get_sim_start = exposed(get_sim_start)
    
    def get_sim_end(self):
        tmp = datetime.fromtimestamp(self.sim_end)
        fmtdate = tmp.strftime("%Y_%m_%d")
        return fmtdate
    get_sim_end = exposed(get_sim_end)

    def get_backfill_count(self):
        return self.backfill_count

    def incr_backfill_count(self):
        self.backfill_count += 1
        return self.backfill_count
    incr_backfill_count = exposed(incr_backfill_count)
