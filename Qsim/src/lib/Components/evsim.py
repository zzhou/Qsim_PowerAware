...
from Cobalt.Components.pwmonitor import PowerMonitor
...
class EventSimulator(Component):
    ...
    def init_power_monitor_events(self):
        """add power monitor points into time stamps"""
        if not self.event_list:
            return
        ...
        powmon_point = first_time_sec + POWMON_INTERVAL
        while powmon_point < last_time_sec:
            evspec = {}
            evspec['machine'] = machine_id
            evspec['unixtime'] = powmon_point
            evspec['datetime'] = sec_to_date(powmon_point)
            self.add_event(evspec)
            powmon_point += POWMON_INTERVAL
        
    init_iomon_events = exposed(init_power_monitor_events)    
...
#                                            )
        ...
        if machine == INTREPID:
            self.powermonitor.monitor_power()
            self.bgsched.schedule_jobs()
            self.last_schedule_time = self.get_current_time()
        if machine == EUREKA:
            self.csched.schedule_jobs()
        if machine == POWMON:
            self.powermonitor.monitor_power(POWMON_INTERVAL)
        
        if self.go_next:
            ComponentProxy("queue-manager").calc_loss_of_capacity()
    
    def print_power_consumption_cost(self):
        print "Total energy cost: ", self.powermonitor.get_cost()
        
    def print_schedule_info(self):
        print "Schedule count: %d, Refill count: %d" % (self.bgsched.get_schedule_count(), self.bgsched.get_refill_count())
        
    def log_power_consumption(self):
        self.powermonitor.log_power_consumption()
    
    def get_last_schedule_time(self):
        return self.last_schedule_time
    get_last_schedule_time = exposed(get_last_schedule_time)
            
    def init_last_schedule_time(self):
        self.last_schedule_time = self.event_list[1].get("unixtime")